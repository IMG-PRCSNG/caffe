...
void TransposeLayer<Ftype, Btype>::LayerSetUp(const vector<Blob*>& bottom,
...
template <typename Ftype, typename Btype>
void TransposeLayer<Ftype, Btype>::Reshape(const vector<Blob*>& bottom,
      const vector<Blob*>& top) {
...
template <typename Ftype, typename Btype>
vector<int> TransposeLayer<Ftype, Btype>::permute(const vector<int>& vec) {
...
template <typename Ftype, typename Btype>
void TransposeLayer<Ftype, Btype>::Forward_cpu(const vector<Blob*>& bottom, 
		const vector<Blob*>& top) {
	transpose_cpu<Ftype>(bottom[0]->count(), bottom[0]->cpu_data<Ftype>(), top[0]->mutable_cpu_data<Ftype>(), 
...
INSTANTIATE_CLASS_FB(TransposeLayer);
