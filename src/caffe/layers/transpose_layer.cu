...
void TransposeLayer<Ftype, Btype>::Forward_gpu(const vector<Blob*>& bottom, 
		const vector<Blob*>& top) {
	const int nthreads = bottom[0]->count();

	transpose_gpu<Ftype>  // NOLINT_NEXT_LINE(whitespace/operators)
...
INSTANTIATE_LAYER_GPU_FORWARD_ONLY_FB(TransposeLayer);
